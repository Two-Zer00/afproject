      ...
      .then((doc) => {
        if (doc.exists) {
          ...
          audio.addEventListener("timeupdate", () => {
            progresss.value = secToPorcentage(
              audio.currentTime,
              audio.duration
            );
            document.getElementById("currentTime").textContent =
              audio.currentTime.toString().toHHMMSS();
          });
...
audio.addEventListener("ended", () => {
  document.getElementById("playBtn").classList.remove("bi-pause");
  document.getElementById("playBtn").classList.add("bi-play");
});
...
progresss.addEventListener("click", (event) => {
  ...
  audio.currentTime =
    porcentageToSecs(
      pixToPorcentage(x, progresss.offsetWidth),
      audio.duration
    ) - audio.style.margin;
  updatePositionState();
});
progresss.addEventListener("mousemove", (event) => {
  ...
  let temp =
    Math.round(x + 0.25) < event.target.offsetWidth - timerContainer.offsetWidth
